...
const WORKOUT_JS_BUILD = "2026-08-25-fondos-sin-cierre-de-frente";
...
const GROUND_STYLE_COUNTERS = new Set(["squat", "crunch", "legraise", "situp", "scissor", "doublecrunch", "pushup", "dip", "inclinepushup"]);
...
// suelo — eso levanta la cadera por encima de los hombros, así que el
// cuerpo queda en una línea ASCENDENTE de las manos a los pies, no la
// línea horizontal ("tumbado boca abajo") que processPushup exige.
//
// Por eso processPushup no vale tal cual para esto: usa
// tiltFromHorizontal(hombro, cadera) con un TECHO para decidir "¿está
// tumbado?" (ON_GROUND_MAX_TILT_DEG, 40°) y otro techo, más laxo, para
// decidir "te has puesto de pie" (PUSHUP_BROKEN_TILT_DEG, 60°) — los dos
// cortarían el contador en cuanto los pies estuvieran razonablemente
// altos, que es justo lo que se pide evitar aquí: "da igual lo alto que
// ponga los pies".
//
// processInclinePushup usa la misma inclinación pero AL REVÉS: en vez de
// un techo, un SUELO (INCLINE_PUSHUP_MIN_TILT_DEG) — por debajo de eso ya
// es, en la práctica, una flexión plana normal, no inclinada, así que no
// arma el contador. Por ENCIMA de ese mínimo no hay techo: por mucho que
// se eleven los pies, sigue contando igual.
//
// Coste de ese diseño, asumido a propósito: sin techo de inclinación,
// esta función no puede distinguir "pies muy altos" de "te has puesto de
// pie" (de pie, el ángulo hombro-cadera respecto a la horizontal también
// sale alto, cerca de 90°). Por eso aquí NO existe el cierre automático
// de serie por romper la postura que sí tiene processPushup (el bloque de
// PUSHUP_BROKEN_TILT_DEG/PUSHUP_BREAK_STABLE_MS): terminar la serie
// depende solo del gesto de la mano o de salir del encuadre (igual que
// sentadillas o abdominales) — ponerte de pie no sirve como señal aquí.
//
// El resto de condiciones (ángulo de codo para contar, línea hombro-
// cadera-tobillo recta para confirmar que el cuerpo no está encogido,
// visibilidad mínima) son EXACTAMENTE las de processPushup — mismo gesto
// de brazo, así que se reutilizan las mismas constantes (PUSHUP_UP_ANGLE_DEG,
// PUSHUP_DOWN_ANGLE_DEG, PUSHUP_MIN_VISIBILITY, PUSHUP_LINE_MIN_DEG,
// ON_GROUND_STABLE_MS) en vez de duplicarlas con otro nombre.
const INCLINE_PUSHUP_MIN_TILT_DEG = 15; // valor de partida, sin probar en cámara real: por debajo de esto se considera flexión plana, no inclinada. Si con los pies apenas elevados (silla baja) cuesta armar el contador, bajarlo; si una flexión plana normal se cuela como "inclinada", subirlo
...
class WorkoutSession {
   ...
   * el bloque INCLINE_PUSHUP_* más arriba para el porqué de cada
   * diferencia con processPushup. En corto: se exige una inclinación
   * MÍNIMA (INCLINE_PUSHUP_MIN_TILT_DEG) en vez del techo de
   * ON_GROUND_MAX_TILT_DEG, sin techo por arriba — y por eso no hay
   * cierre de serie automático por "te has puesto de pie" (esa señal no
   * sirve aquí): solo el gesto de la mano o salir del encuadre terminan
   * la serie.
   */
  processInclinePushup(lm, now) {
    ...
    // Sin bloque de "romper postura": a diferencia de processPushup, aquí
    // no hay techo de inclinación con el que detectar que te has puesto
    // de pie (ver el porqué en el comentario del bloque INCLINE_PUSHUP_*).
    // La serie se cierra solo por el gesto de la mano (arriba) o por salir
    // del encuadre (GROUND_STYLE_COUNTERS, en processResult).
    ...
    if (this.state === null) {
      ...
      const bodyStraight = lineAngle >= PUSHUP_LINE_MIN_DEG;
      const armsStraight = elbowAngle >= PUSHUP_UP_ANGLE_DEG;
      const inclined = tilt >= INCLINE_PUSHUP_MIN_TILT_DEG;

      if (inclined && bodyStraight && armsStraight) {
        if (this.groundStableSince === null) this.groundStableSince = now;
        if (now - this.groundStableSince >= ON_GROUND_STABLE_MS) {
          this.state = "top";
          this.groundStableSince = null;
        ...
        this.groundStableSince = null;
        this.setStatus(
          "Túmbate boca abajo con los pies apoyados en alto (una silla, un escalón, un sofá…), de perfil " +
          "a la cámara, con los brazos estirados, las manos a la altura del pecho y los codos pegados al " +
          "cuerpo (mirando hacia atrás), para empezar."
        );
      }
    } else if (this.state === "top") {
      if (elbowAngle <= PUSHUP_DOWN_ANGLE_DEG) {
    ...
    if (this.debugEl) {
      this.debugEl.textContent =
        `ángulo codo (${useLeft ? "izq" : "der"}): ${elbowAngle.toFixed(0)}° | inclinación: ${tilt.toFixed(0)}° | estado: ${this.state ?? "esperando"} ` +
        `(mínimo inclinación: ${INCLINE_PUSHUP_MIN_TILT_DEG}°, abajo ≤${PUSHUP_DOWN_ANGLE_DEG}°, arriba ≥${PUSHUP_UP_ANGLE_DEG}°)`;
    }
  }
